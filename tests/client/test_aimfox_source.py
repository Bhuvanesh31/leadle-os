...
def test_read_builds_campaign_with_variant_and_metrics():
    m = {
        "/campaigns": {
            "campaigns": [{"id": "a1", "name": "Upsta_US_PMP_V1"}, {"id": "z9", "name": "Other_V1"}]
        },
        "/campaigns/a1": {
            "campaign": {
                "flows": [
                    {
                        ...
                        "template": {"message": "Hi {{FIRST_NAME}}, founder"},
                    }
                ]
            }
        },
    ...
    camps = aimfox_source.read(
        "KEY", ("2026-05-01", "2026-07-01"), name_contains="upsta", client=client
    )
    assert len(camps) == 1
    c = camps[0]
