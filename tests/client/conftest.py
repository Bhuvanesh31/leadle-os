...
def make_data() -> ClientData:
    ...
    return ClientData(
        ...
        linkedin_campaigns=[
            LinkedInCampaign(name="Founder intro", invites=120, accepted=9, replied=3,
                             variant_message="I'm the founder at Acme"),
            LinkedInCampaign(name="CFO angle", invites=80, accepted=5, replied=0,
                             variant_message="we work with finance leaders"),
        ],
        replies=[
            ReplyRecord(channel="linkedin", campaign="Founder intro", sentiment="neutral", name="Bob"),
            ReplyRecord(channel="email", campaign="A<b", sentiment="positive", name="Alice"),
        ],
        warm_leads=[
            WarmLead(
                channel="LinkedIn", account="ACME", response_date="2026-06-01",
                status="Meeting booked", response_text="Let's connect.",
                linkedin_url="https://linkedin.com/in/carol", name="Carol",
                title="VP Finance", company="Acme", company_url="https://acme.com",
                location="New York",
            ),
        ],
        senders=[
        ...
        targets=[
            ...
            TargetCo(
                name="Globex", country="SG", location="Singapore",
                linkedin_url="https://linkedin.com/company/globex",
                industry="Fintech", size="11-50", segment="ICP-B",
                domain="globex.com", aimfox_id="ax-002", instantly_id="",
            ),
        ],
    )
...
def make_data_zero_opens() -> ClientData:
    ...
    return ClientData(
        email_campaigns=[EmailCampaign(name="C1", sent=50, opened=0, clicked=0, bounced=1, replied=0)],
...
def _render(data: ClientData, audience: str) -> str:
    ...
    dbag = {"emails_sent": {"value": metrics["kpis"]["emails_sent"],
                            "delta": None, "baseline": True}}
    return render.render(
        data, metrics, dbag,
        {"narrative": "June ran hot on volume.", "degraded": False},
        {"actions": ["Pause & warm the augustine inbox."], "degraded": False},
        audience=audience, period_label="June 2026", client="UPSTA",
        layout=layout, rubric=rubric,
    )
