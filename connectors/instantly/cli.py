...
Reads INSTANTLY_API_KEY from env (or .env in repo root), accepts --start/--end
...
def main(argv: list[str] | None = None) -> int:
    load_dotenv()
    parser = argparse.ArgumentParser(prog="connectors.instantly.cli")
    ...
    parser.add_argument(
        "--name-contains",
        default=None,
        help="Case-insensitive substring filter on campaign name (e.g. 'Upsta')",
    )
    args = parser.parse_args(argv)

    api_key = os.environ.get("INSTANTLY_API_KEY")
    if not api_key:
        result = {"available": False, "reason": "INSTANTLY_API_KEY not set in environment"}
    else:
        result = fetch(
            api_key,
            args.start,
